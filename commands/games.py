...
class GameCommands(app_commands.Group):
    ...
    async def games_dice(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="The dice have been thrown!",
    ...
    @app_commands.describe(item="rock, paper or scissors")
    ...
    async def dice(self, interaction: discord.Interaction, item: discord.app_commands.Choice[str]):
        bot_item = random.choice(["rock", "paper", "scissors"])
        outcomes = {
        ...
        if bot_item == item.name:
            ...
            await interaction.response.send_message(embed=embed)
        elif bot_item == outcomes[item.name]["win"]:
            embed = discord.Embed(title=":video_game: Game: rock, paper, scissors", colour=discord.Colour(0x00ff00),
    ...
    @app_commands.command(name="connect4", description="Play connect4 against another user or AI")
